...
if not exist "%BUILD_VENV%\Scripts\python.exe" (
  echo [1/9] Creating build environment...
...
echo [2/9] Installing/updating build tools...
...
echo [4/9] Checking Python compilation...
python -m compileall -q .
if errorlevel 1 goto :fail

echo [5/9] Checking desktop UI startup...
python main.py --startup-check
if errorlevel 1 goto :fail

echo [6/9] Cleaning old desktop build...
...
echo [8/9] Smoke-testing packaged EXE...
start "" /wait "dist\%APP_NAME%\%APP_NAME%.exe" --startup-check
if errorlevel 1 goto :fail
...
echo [9/9] Creating portable ZIP...
